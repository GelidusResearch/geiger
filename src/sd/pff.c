...
#define IsDBCS2(c)                                                                                                     \
	(((uint8_t)(c) >= _DS1S && (uint8_t)(c) <= _DS1E) || ((uint8_t)(c) >= _DS2S && (uint8_t)(c) <= _DS2E)                          \
...
#if _FS_FAT16
	case FS_FAT16:
		if (disk_readp(buf, fs->fatbase + clst / 256, ((uint16_t)clst % 256) * 2, 2))
			break;
		return LD_WORD(buf);
#endif
#if _FS_FAT32
	case FS_FAT32:
		if (disk_readp(buf, fs->fatbase + clst / 128, ((uint16_t)clst % 128) * 4, 4))
			break;
...
static CLUST get_clust(uint8_t *dir // Pointer to directory entry
                       )
{
	FATFS *fs   = FatFs;
	CLUST clst = 0;
...
static FRESULT dir_find(DIR * dj, uint8_t *dir )
{
	...
	do {
		...
		if (!(dir[DIR_Attr] & ATTRIB_VOL) && !mem_cmp(dir, dj->fn, 11))			// Is it a valid entry?
			break;
...
static FRESULT dir_read(DIR * dj, uint8_t *dir )
{
	...
	res = FR_NO_FILE;
	while (dj->sect) {
		res = disk_readp(dir, dj->sect, (dj->index % 16) * 32, 32) ? FR_DISK_ERR : FR_OK;	// Read an entry
		if (res != FR_OK)
			break;
		c = dir[DIR_Name];
		if (c == 0) {
		...
		a = dir[DIR_Attr] & ATTRIB_MASK;
		if (c != 0xE5 && c != '.' && !(a & ATTRIB_VOL)) // Is it a valid entry?
			break;
		res = dir_next(dj); // Next entry
		if (res != FR_OK)
			break;
...
// Follow a file path
//-----------------------------------------------------------------------
// Returns: FR_OK(0): successful, !=0: error code
// Parms:
...
static FRESULT follow_path( DIR * dj, uint8_t * dir, const char *path )
{
	...
	if ((uint8_t)*path < ' ') { // Null path means the root directory
		...
		for (;;) {
			...
			if (!(dir[DIR_Attr] & ATTRIB_DIR)) { // Cannot follow path because it is a file
				res = FR_NO_FILE;
				break;
			}
...
FRESULT pf_mount(FATFS *fs )
{
	...
	if (fmt == 3)
		return FR_DISK_ERR;
	if (fmt)
		return FR_NO_FILESYSTEM;						// No valid FAT partition is found
	...
	//if (_FS_FAT12 && mclst < 0xFF7) fmt = FS_FAT12;
	if (_FS_FAT16 && mclst >= 0xFF8 && mclst < 0xFFF7) fmt = FS_FAT16;
	if (_FS_FAT32 && mclst >= 0xFFF7) fmt = FS_FAT32;
...
FRESULT pf_open(const char *path ) // Pointer to the file name
{
	...
	if (!dir[0] || (dir[DIR_Attr] & ATTRIB_DIR)) // It is a directory
		return FR_NO_FILE;
...
#if _USE_WRITE
FRESULT pf_write(const void *buff, uint16_t btw, uint16_t * bw )
{
	CLUST       clst;
	uint32_t       sect, remain;
	...
	if (!fs) { // Check file system
		...
		if (res == FR_OK) {                  // Follow completed
			if (dir[0]) {                    // It is not the root dir
				if (dir[DIR_Attr] & ATTRIB_DIR)  // The object is a directory
